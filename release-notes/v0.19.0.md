...
`playerData.atomicSidecarWrite`（默认开启）改为临时文件 + `fsync` + 原子改名，并在改名前把上一份留作 `.bak`。文件系统不支持原子改名时自动回退
...
- 单元测试 429 项全部通过（公共模块 69 + Forge 225 + NeoForge 135），其中本版新增 39 项
- 对每一处新增逻辑做了变异检查：移除备份复位逻辑、移除备份轮转、移除脏标志判断、移除 `DataVersion` 判据后，相应用例均如期失败
