...
public final class TickGapDetector {
    ...
    /**
     * 剥掉 Java 21 lambda 隐藏类名里 '/' 之后的那串地址 —— 它每次启动都不同, 留着会让同一个 lambda
    ...
    /**
     * 复位限流状态与抑制计数。由 {@code /betterautosave diagnose reset} 与
     * {@code BetterAutoSaveCore.uninstall()} 调用 —— 统计被清空之后, 下一次超阈值理应能重新打出
     * 那一行人类可读的证据, 否则 reset 之后的日志是哑的。
     */
