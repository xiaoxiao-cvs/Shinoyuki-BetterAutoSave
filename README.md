...
把构建产物 `build/libs/shinoyuki_betterautosave-0.9.0.jar` 放进服务端 `mods/` 目录, 启动后配置文件生成在 `config/Shinoyuki-Optimize/shinoyuki_betterautosave/common.toml`。所有 Shinoyuki 系列优化 mod 共享 `config/Shinoyuki-Optimize/` 父目录, 每 mod 一个子文件夹便于集中管理。
...
| prometheus.enabled | false | v0.9: 启用 Prometheus metrics HTTP exporter (opt-in) |
| prometheus.bindAddress | "0.0.0.0" | v0.9: HTTP server 绑定地址, 公网服请用防火墙限制或改 127.0.0.1 |
| prometheus.port | 9450 | v0.9: HTTP server 端口, 避开 9090/9100/25565 |
| hottestChunks.windowSize | 100 | v0.9: per-chunk 滑动窗口样本数, p99 计算用 |
| hottestChunks.trackLimit | 10000 | v0.9: LRU 跟踪上限, 触顶时 evict 最久未 save 的 chunk |
...
- `/betterautosave hottest-chunks [count]` — v0.9: 列出滑动窗口内 worker NBT build p99 最高的 top N chunk (默认 10, 最多 50), 定位单点慢 chunk

## v0.9 工具化与监控

v0.9 新增两条诊断手段, 让服主自助定位 mod / vanilla 瓶颈, 不再依赖外接 spark profiler.

### Prometheus exporter

启用后 BAS 在 `prometheus.bindAddress:prometheus.port/metrics` 暴露 Prometheus exposition format text. 配 Grafana 可做生产长时间监控, 历史趋势比 `/betterautosave debug` 单点快照诊断价值更高.

启用方式: `config/Shinoyuki-Optimize/shinoyuki_betterautosave/common.toml`:
...
暴露指标 (全部用 `bas_` 前缀避免跟系列其他 mod 命名冲突):

- 17 个 counter: chunks/entities/savedData submitted/completed/failed/retried/fallback + chunkMapSave async/fallback/bypass
- 5 个 gauge: mustDrainPending / workerQueueDepth / entityQueueDepth / inFlightSerializing / inFlightIoPending
- 4 个 histogram: mainThreadCapture / workerNbtBuild / ioStore / eventDispatch (单位秒, bucket 已扩到 60s)

实现细节: JDK 内置 `HttpServer` (零外部依赖), 单 daemon 线程 executor, 启动失败 log error 不 crash 服, lifecycle 跟随 onServerStarting/Stopping.

**安全提醒**: bindAddress 默认 `0.0.0.0` 是为 onboarding 摩擦小. 公网服 (云服务器 / VPS) 必须用防火墙 (Security Group / iptables) 限制 9450 端口访问, 或改 `127.0.0.1` 仅本地抓取. metrics 数据虽不直接敏感, 但暴露世界活动模式.

### hottest-chunks 命令

`/betterautosave hottest-chunks [count]` 列出 `ChunkLatencyTracker` 滑动窗口内 worker NBT build p99 最高的 top N chunk:
...
数据来源: `ChunkSaveTask.execute` 在 `ChunkNbtAssembler.assemble` 前后 nanoTime 测耗时回写 tracker. tracker 内部 ring buffer 滑动窗口 + LinkedHashMap (access-order) LRU eviction, 内存占用 ~1.6 KB × trackLimit (默认 10k = ~16 MB).

应用场景: 识别哪些 chunk 是 BlockEntity 多 / structure 复杂的 hot path, 排查路径包括:
- 单个 chunk 内有数百个 BlockEntity (大型自动化农场 / mod 商店面板)
- 复杂 structure NBT (大型船坞 / 地下城)
- `count` 远小于 `windowSize` 但 p99 高: 该 chunk 罕见 save 但每次都贵
...
- SavedData 路径接管 (`DimensionDataStorage.save` HEAD 拦截, 主线程构 tag 后异步写盘 + 大文件 fallback)
- SaveListener API (`SaveListenerRegistry`) — chunk / entity / SavedData 三类公开订阅接口, 解锁 BetterBackup 等下游 mod
- **v0.9 Prometheus metrics HTTP exporter (`bas_` 前缀, 17 counter / 5 gauge / 4 histogram)**
- **v0.9 `/betterautosave hottest-chunks` 命令 (滑动窗口 + LRU 跟踪 per-chunk worker latency)**
- mustDrain 状态机 (chunk + entity 共用 mustDrainPending gauge) + 关服 shutdownMode 守卫
- `/betterautosave debug / metrics / flush / status / force-async / drain-unload / hottest-chunks` 命令套件 (全部异步, 主线程 0 阻塞)
...
构建产物: `build/libs/shinoyuki_betterautosave-0.9.0.jar`
...
2. **完全卸载**: 把 `shinoyuki_betterautosave-0.9.0.jar` 从 `mods/` 移走重启即可。world data 由 vanilla autosave 持续保护, 卸载不会丢数据
