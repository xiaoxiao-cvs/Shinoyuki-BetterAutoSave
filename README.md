...
把构建产物 `build/libs/shinoyuki_betterautosave-0.7.0.jar` 放进服务端 `mods/` 目录, 启动后配置文件生成在 `config/Shinoyuki-Optimize/shinoyuki_betterautosave/common.toml`。所有 Shinoyuki 系列优化 mod 共享 `config/Shinoyuki-Optimize/` 父目录, 每 mod 一个子文件夹便于集中管理。
...
构建产物: `build/libs/shinoyuki_betterautosave-0.7.0.jar`
...
2. **完全卸载**: 把 `shinoyuki_betterautosave-0.7.0.jar` 从 `mods/` 移走重启即可。world data 由 vanilla autosave 持续保护, 卸载不会丢数据
