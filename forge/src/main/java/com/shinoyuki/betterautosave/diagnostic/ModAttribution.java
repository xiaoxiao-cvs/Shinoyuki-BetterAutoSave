 ...
 *
 * <p>刻意不用 {@code Class#getProtectionDomain().getCodeSource()} 做归因: Forge 1.20.1 的 SecureJar
 * 模块类加载器下该 URL 的形态未经核实, 而 ModList 的 scan-data 反查链每一个方法签名都已核实存在。
 */
public final class ModAttribution {
    ...
    /**
     * 命中返回 modid, 未命中返回 null。调用方必须自己兜底成 FQCN —— 索引构建失败 (例如未来 Forge 换掉
     * scan-data API) 时本方法恒返回 null, 归因降级为全限定类名, 主流程不受影响。
     */
    ...
    private static synchronized Map<String, String> buildIndex() {
        ...
        try {
            ...
            // 保留已经建好的那部分索引: 中途失败 (例如第 200 个 mod 的 scan data 有问题) 时,
            // 前面所有 mod 的归因仍然可用。仍然置入 INDEX 而不留 null, 因此不会重试, 也就不会
            // 每次超阈值都重跑一遍失败的全量扫描。
