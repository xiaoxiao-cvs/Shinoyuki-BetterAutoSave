...
一处需要知道的失效模式：同时装了重写区块获取路径的 mod（C2ME 一类）时，被检测的那次调用可能已经不存在，探针装不上去，同步加载检测不生效。判断依据是 `bas_sync_load_stalls_total` 长期为 0 而服务器确实在卡。

其中有一类会明确报出来。Lithium 系移植（Harium / Radium / Canary）用 `@Overwrite` 整体接管了 `ServerChunkCache.getChunk`，自 v0.20.1 起 BAS 在启动期检测到它们就跳过这个 mixin，并打一行 WARN 指明是哪个 mod 触发的。v0.20.0 曾试图只靠注入器上的 `require = 0` 兜住这种情况，但 Mixin 的优先级判定发生在 require 检查之前，实际结果是启动崩溃而非静默跳过；机制与完整名单见 `docs/COMPATIBILITY.md` 的「同步加载检测器的启动期门控」。
