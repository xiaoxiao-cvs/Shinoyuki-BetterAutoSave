# 归档：BetterAutoSave v0.2 - v0.9 开发记录

> 本文是历史归档，不再维护。记录 BAS 早期（v0.2 至 v0.9）逐版本的技术方案、实施偏差与生产实测数据。
>
> 其中的实测数据（spark 采样、MSPT spike 元凶分析、worker 线程负载）来自 2026-05 前后的生产环境，采集条件写在各节内，做后续决策时可作参考基线，但不代表当前版本的行为。
>
> 当前版本的路线图与能力总览见 [ROADMAP.md](../ROADMAP.md)，兼容性见 [COMPATIBILITY.md](../COMPATIBILITY.md)，配置说明见 [CONFIGURATION.md](../CONFIGURATION.md)。
